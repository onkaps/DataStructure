...
int IntBST_Height_calculate(IntBSTNode *p)
{
    int x, y;
    if (p == NULL)
        return 0;
    ...
    {
        x = IntBST_Height_calculate(p->lchild);
        y = IntBST_Height_calculate(p->rchild);
        if (x > y)
            return x + 1;
        else
            return y + 1;
...
void IntBST_Height(IntBSTNode *p)
{
    printf("\nHeight of BST : %d\n", IntBST_Height_calculate(Int_BST_root));
}
...
IntBSTNode *IntBST_Delete(IntBSTNode *p, int key)
{
    ...
    {
        if (IntBST_Height_calculate(p->lchild) > IntBST_Height_calculate(p->rchild))
        {
